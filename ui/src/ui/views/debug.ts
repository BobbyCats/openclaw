...
export function renderDebug(props: DebugProps) {
  ...
  const securityLabel =
    critical > 0
      ? t("debug.critical", { count: critical })
      : warn > 0
        ? t("debug.warnings", { count: warn })
        : t("debug.noCritical");
  const infoSuffix = info > 0 ? t("debug.info", { count: info }) : "";
      ...
      <div class="card">
        <div class="row" style="justify-content: space-between;">
          ...
          <button class="btn" ?disabled=${props.loading} @click=${props.onRefresh}>
            ${props.loading ? t("debug.refreshing") : t("debug.refresh")}
          </button>
        </div>
            ...
            ${
              securitySummary
                ? html`<div class="callout ${securityTone}" style="margin-top: 8px;">
                  ${t("debug.securityAudit", { label: securityLabel, info: infoSuffix })}
                  <span class="mono">openclaw security audit --deep</span> ${t("debug.securityAuditCmd")}
                </div>`
                : nothing
            }
