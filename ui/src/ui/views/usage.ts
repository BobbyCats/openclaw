      ...
      <div class="card usage-mosaic">
        <div class="usage-mosaic-header">
          <div>
            ...
            <div class="usage-mosaic-sub">${t("usage.estimatesRequireTimestamps")}</div>
          </div>
    ...
    <div class="card usage-mosaic">
      <div class="usage-mosaic-header">
        <div>
          ...
          <div class="usage-mosaic-sub">
            ${t("usage.estimatedFromSpans", { zone: timeZone === "utc" ? "UTC" : t("usage.local") })}
          </div>
        </div>
      ...
      <div class="usage-mosaic-grid">
        <div class="usage-mosaic-section">
          <div class="usage-mosaic-section-title">${t("usage.dayOfWeek")}</div>
        ...
        <div class="usage-mosaic-section">
          ...
          <div class="usage-hour-labels">
            ...
            <span>${t("usage.fourAm")}</span>
            <span>${t("usage.eightAm")}</span>
            <span>${t("usage.noon")}</span>
            <span>${t("usage.fourPm")}</span>
            <span>${t("usage.eightPm")}</span>
          </div>
          <div class="usage-hour-legend">
            <span></span>
            ${t("usage.tokenDensityLegend")}
          </div>
        </div>
      </div>
    </div>
...
function renderFilterChips(
  ...
  const sessionsLabel = selectedSession
    ...
    : selectedSessions.length === 1
      ...
      : t("usage.nSessions", { count: selectedSessions.length });
  ...
  const daysLabel =
    selectedDays.length === 1 ? selectedDays[0] : t("usage.nDays", { count: selectedDays.length });
  const hoursLabel =
    selectedHours.length === 1
      ? `${selectedHours[0]}:00`
      : t("usage.nHours", { count: selectedHours.length });
    ...
    <div class="active-filters">
        ...
        selectedDays.length > 0
          ? html`
            <div class="filter-chip">
              <span class="filter-chip-label">${t("usage.daysFilter", { label: daysLabel })}</span>
      ...
      ${
        selectedHours.length > 0
          ? html`
            <div class="filter-chip">
              <span class="filter-chip-label">${t("usage.hoursFilter", { label: hoursLabel })}</span>
        ...
        selectedSessions.length > 0
          ? html`
            <div class="filter-chip" title="${sessionsFullName}">
              <span class="filter-chip-label">${t("usage.sessionFilter", { label: sessionsLabel })}</span>
              ...
              dailyChartMode === "by-type"
                ? isTokenMode
                  ? [
                      ...
                      `${t("usage.cacheWriteLower")} ${formatTokens(d.cacheWrite)}`,
                      `${t("usage.cacheReadLower")} ${formatTokens(d.cacheRead)}`,
                    ]
                  : [
                      `${t("usage.output")} ${formatCost(d.outputCost ?? 0)}`,
                      `${t("usage.input")} ${formatCost(d.inputCost ?? 0)}`,
                      `${t("usage.cacheWriteLower")} ${formatCost(d.cacheWriteCost ?? 0)}`,
                      `${t("usage.cacheReadLower")} ${formatCost(d.cacheReadCost ?? 0)}`,
                    ]
                : [];
...
function renderCostBreakdownCompact(totals: UsageTotals, mode: "tokens" | "cost") {
  ...
  return html`
    <div class="cost-breakdown cost-breakdown-compact">
      <div class="cost-breakdown-header">${isTokenMode ? t("usage.tokensByType") : t("usage.costByType")}</div>
      <div class="cost-breakdown-bar">
        <div class="cost-segment output" style="width: ${(isTokenMode ? tokenPcts.output : breakdown.output.pct).toFixed(1)}%"
          title="${t("usage.output")}: ${isTokenMode ? formatTokens(totals.output) : formatCost(breakdown.output.cost)}"></div>
        <div class="cost-segment input" style="width: ${(isTokenMode ? tokenPcts.input : breakdown.input.pct).toFixed(1)}%"
          title="${t("usage.input")}: ${isTokenMode ? formatTokens(totals.input) : formatCost(breakdown.input.cost)}"></div>
        <div class="cost-segment cache-write" style="width: ${(isTokenMode ? tokenPcts.cacheWrite : breakdown.cacheWrite.pct).toFixed(1)}%"
          title="${t("usage.cacheWrite")}: ${isTokenMode ? formatTokens(totals.cacheWrite) : formatCost(breakdown.cacheWrite.cost)}"></div>
        <div class="cost-segment cache-read" style="width: ${(isTokenMode ? tokenPcts.cacheRead : breakdown.cacheRead.pct).toFixed(1)}%"
          title="${t("usage.cacheRead")}: ${isTokenMode ? formatTokens(totals.cacheRead) : formatCost(breakdown.cacheRead.cost)}"></div>
      </div>
      ...
      <div class="usage-summary-grid">
        <div class="usage-summary-card">
          ...
          <div class="usage-summary-sub">
            ${aggregates.messages.user} ${t("usage.userLabel")} · ${aggregates.messages.assistant} ${t("usage.assistantLabel")}
          </div>
        </div>
        <div class="usage-summary-card">
          <div class="usage-summary-title">
          ...
          <div class="usage-summary-sub">${t("usage.toolsUsed", { count: aggregates.tools.uniqueTools })}</div>
        </div>
        <div class="usage-summary-card">
          <div class="usage-summary-title">
            ${t("usage.errors")}
          ...
          <div class="usage-summary-sub">${t("usage.toolResults", { count: aggregates.messages.toolResults })}</div>
        </div>
        <div class="usage-summary-card">
          <div class="usage-summary-title">
          ...
          <div class="usage-summary-sub">${t("usage.acrossNMessages", { count: aggregates.messages.total || 0 })}</div>
        </div>
        <div class="usage-summary-card">
          <div class="usage-summary-title">
          ...
          <div class="usage-summary-sub">${t("usage.totalCost", { cost: formatCost(totals.totalCost) })}</div>
        </div>
        <div class="usage-summary-card">
          <div class="usage-summary-title">
            ${t("usage.sessionsLabel")}
            <span class="usage-summary-hint" title="${t("usage.sessionsHint")}">?</span>
          </div>
          <div class="usage-summary-value">${sessionCount}</div>
          <div class="usage-summary-sub">${t("usage.ofNInRange", { total: totalSessions })}</div>
        </div>
        <div class="usage-summary-card">
          <div class="usage-summary-title">
            ${t("usage.throughput")}
        ...
        <div class="usage-summary-card">
          ...
          <div class="usage-summary-sub">
            ${t("usage.errorsAvgSession", { errors: aggregates.messages.errors, duration: avgDurationLabel })}
          </div>
        </div>
        <div class="usage-summary-card">
          <div class="usage-summary-title">
          ...
          <div class="usage-summary-sub">
            ${t("usage.cachedPrompt", { cached: formatTokens(totals.cacheRead), prompt: formatTokens(cacheBase) })}
          </div>
        </div>
      </div>
  ...
  return html`
    <div class="card sessions-card">
      <div class="sessions-card-header">
        <div class="card-title">${t("usage.sessionsLabel")}</div>
        <div class="sessions-card-count">
          ${totalSessions !== sessions.length ? t("usage.shownTotal", { count: sessions.length, total: totalSessions }) : t("usage.shown", { count: sessions.length })}
        </div>
      </div>
      <div class="sessions-card-meta">
        <div class="sessions-card-stats">
          <span>${isTokenMode ? formatTokens(avgValue) : formatCost(avgValue)} ${t("usage.avg")}</span>
          <span>${totalErrors} ${t("usage.errors").toLowerCase()}</span>
        </div>
        ...
        <label class="sessions-sort">
          ...
          >
            <option value="cost" ?selected=${sessionSort === "cost"}>${t("usage.cost")}</option>
            <option value="errors" ?selected=${sessionSort === "errors"}>${t("usage.errors")}</option>
            <option value="messages" ?selected=${sessionSort === "messages"}>${t("usage.messages")}</option>
            <option value="recent" ?selected=${sessionSort === "recent"}>${t("usage.recent")}</option>
            <option value="tokens" ?selected=${sessionSort === "tokens"}>${t("usage.tokens")}</option>
          </select>
        </label>
        <button
          ...
          : sessions.length === 0
            ...
            : html`
                <div class="session-bars">
                  ...
                  ${sessions.length > 50 ? html`<div class="muted" style="padding: 8px; text-align: center; font-size: 11px;">${t("usage.nMore", { count: sessions.length - 50 })}</div>` : nothing}
                </div>
              `
      }
      ${
        selectedCount > 1
          ? html`
              <div style="margin-top: 10px;">
                <div class="sessions-card-count">${t("usage.selectedN", { count: selectedCount })}</div>
...
function renderSessionSummary(session: UsageSessionEntry) {
  ...
  if (!usage) {
    return html`
      <div class="muted">${t("usage.noUsageData")}</div>
    `;
  }
  ...
  return html`
    ...
    <div class="session-summary-grid">
      <div class="session-summary-card">
        <div class="session-summary-title">${t("usage.messages")}</div>
        <div class="session-summary-value">${usage.messageCounts?.total ?? 0}</div>
        <div class="session-summary-meta">${usage.messageCounts?.user ?? 0} ${t("usage.userLabel")} · ${usage.messageCounts?.assistant ?? 0} ${t("usage.assistantLabel")}</div>
      </div>
      <div class="session-summary-card">
        <div class="session-summary-title">${t("usage.toolCalls")}</div>
        <div class="session-summary-value">${usage.toolUsage?.totalCalls ?? 0}</div>
        <div class="session-summary-meta">${t("usage.toolsUsed", { count: usage.toolUsage?.uniqueTools ?? 0 })}</div>
      </div>
      <div class="session-summary-card">
        <div class="session-summary-title">${t("usage.errors")}</div>
        <div class="session-summary-value">${usage.messageCounts?.errors ?? 0}</div>
        <div class="session-summary-meta">${t("usage.toolResults", { count: usage.messageCounts?.toolResults ?? 0 })}</div>
      </div>
      <div class="session-summary-card">
      ...
      <div class="session-detail-header">
        ...
        <div class="session-detail-stats">
          ${
            usage
              ? html`
            <span><strong>${formatTokens(usage.totalTokens)}</strong> ${t("usage.tokensUnit")}</span>
            <span><strong>${formatCost(usage.totalCost)}</strong></span>
          `
              : nothing
          }
        </div>
        <button class="session-close-btn" @click=${onClose} title="${t("usage.closeSessionDetails")}">×</button>
      </div>
...
function renderTimeSeriesCompact(
  ...
  if (loading) {
    return html`
      <div class="session-timeseries-compact">
        <div class="muted" style="padding: 20px; text-align: center">${t("usage.loadingEllipsis")}</div>
      </div>
    `;
  }
  if (!timeSeries || timeSeries.points.length < 2) {
    return html`
      <div class="session-timeseries-compact">
        <div class="muted" style="padding: 20px; text-align: center">${t("usage.noTimelineData")}</div>
      </div>
    `;
  }
  ...
  if (points.length < 2) {
    return html`
      <div class="session-timeseries-compact">
        <div class="muted" style="padding: 20px; text-align: center">${t("usage.noDataInRange")}</div>
      </div>
    `;
  }
  let cumTokens = 0,
  ...
  return html`
    <div class="session-timeseries-compact">
      <div class="timeseries-header-row">
        <div class="card-title" style="font-size: 13px;">${t("usage.usageOverTime")}</div>
      ...
      <svg viewBox="0 0 ${width} ${height + 15}" class="timeseries-svg" style="width: 100%; height: auto;">
        ...
        ${points.map((p, i) => {
          ...
          const tooltipLines = [
            ...
            `${formatTokens(val)} ${t("usage.tokensUnit")}`,
          ];
          if (breakdownByType) {
            tooltipLines.push(`${t("usage.output")} ${formatTokens(p.output)}`);
            tooltipLines.push(`${t("usage.input")} ${formatTokens(p.input)}`);
            tooltipLines.push(`${t("usage.cacheWriteLower")} ${formatTokens(p.cacheWrite)}`);
            tooltipLines.push(`${t("usage.cacheReadLower")} ${formatTokens(p.cacheRead)}`);
          }
      ...
      <div class="timeseries-summary">${points.length} ${t("usage.msgs")} · ${formatTokens(cumTokens)} · ${formatCost(cumCost)}</div>
      ${
        breakdownByType
          ? html`
              <div style="margin-top: 8px;">
                <div class="card-title" style="font-size: 12px; margin-bottom: 6px;">${t("usage.tokensByType")}</div>
                ...
                <div class="cost-breakdown-legend">
                  ...
                  <div class="legend-item" title="${t("usage.tokensWrittenToCache")}">
                    <span class="legend-dot cache-write"></span>${t("usage.cacheWrite")} ${formatTokens(sumCacheWrite)}
                  </div>
                  <div class="legend-item" title="${t("usage.tokensReadFromCache")}">
                    <span class="legend-dot cache-read"></span>${t("usage.cacheRead")} ${formatTokens(sumCacheRead)}
                  </div>
                </div>
                <div class="cost-breakdown-total">${t("usage.totalLabel")} ${formatTokens(totalTypeTokens)}</div>
              </div>
            `
          : nothing
      }
...
function renderContextPanel(
  ...
  if (!contextWeight) {
    return html`
      <div class="context-details-panel">
        <div class="muted" style="padding: 20px; text-align: center">${t("usage.noContextData")}</div>
      </div>
    `;
  }
  ...
  if (usage && usage.totalTokens > 0) {
    ...
    if (inputTokens > 0) {
      contextPct = t("usage.ofInput", {
        pct: Math.min((totalContextTokens / inputTokens) * 100, 100).toFixed(0),
      });
    }
  }
  ...
  return html`
    <div class="context-details-panel">
      <div class="context-breakdown-header">
        <div class="card-title" style="font-size: 13px;">${t("usage.systemPromptBreakdown")}</div>
        ${
          hasMore
            ? html`<button class="context-expand-btn" @click=${onToggleExpanded}>
                ${showAll ? t("usage.collapse") : t("usage.expandAll")}
              </button>`
            : nothing
        }
      </div>
      <p class="context-weight-desc">${contextPct || t("usage.baseContextPerMessage")}</p>
      ...
      <div class="context-legend">
        ...
        <span class="legend-item"><span class="legend-dot skills"></span>${t("usage.skillsLabel")} ~${formatTokens(skillsTokens)}</span>
        <span class="legend-item"><span class="legend-dot tools"></span>${t("usage.toolsLabel")} ~${formatTokens(toolsTokens)}</span>
        <span class="legend-item"><span class="legend-dot files"></span>${t("usage.filesLabel")} ~${formatTokens(filesTokens)}</span>
      </div>
            ...
            ? (() => {
                ...
                return html`
                  <div class="context-breakdown-card">
                    <div class="context-breakdown-title">${t("usage.skillsCount", { count: skillsList.length })}</div>
        ...
        ${
          toolsList.length > 0
            ? (() => {
                ...
                return html`
                  <div class="context-breakdown-card">
                    <div class="context-breakdown-title">${t("usage.toolsCount", { count: toolsList.length })}</div>
                    <div class="context-breakdown-list">
                      ${toolsTop.map(
                        (t) => html`
                          <div class="context-breakdown-item">
        ...
        ${
          filesList.length > 0
            ? (() => {
                ...
                return html`
                  <div class="context-breakdown-card">
                    <div class="context-breakdown-title">${t("usage.filesCount", { count: filesList.length })}</div>
                    <div class="context-breakdown-list">
                      ${filesTop.map(
                        (f) => html`
                          <div class="context-breakdown-item">
...
function renderSessionLogsCompact(
  ...
  if (loading) {
    return html`
      <div class="session-logs-compact">
        <div class="session-logs-header">${t("usage.conversation")}</div>
        <div class="muted" style="padding: 20px; text-align: center">${t("usage.loadingEllipsis")}</div>
      </div>
    `;
  }
  if (!logs || logs.length === 0) {
    return html`
      <div class="session-logs-compact">
        <div class="session-logs-header">${t("usage.conversation")}</div>
        <div class="muted" style="padding: 20px; text-align: center">${t("usage.noMessages")}</div>
      </div>
    `;
  }
  ...
  return html`
    <div class="session-logs-compact">
      <div class="session-logs-header">
        <span>${t("usage.conversation")} <span style="font-weight: normal; color: var(--text-muted);">(${t("usage.messagesCount", { count: displayedCount })})</span></span>
        <button class="btn btn-sm usage-action-btn usage-secondary-btn" @click=${onToggleExpandedAll}>
          ${expandedAll ? t("usage.collapseAll") : t("usage.expandAllCaps")}
        </button>
      </div>
      <div class="usage-filters-inline" style="margin: 10px 12px;">
        ...
        >
          <option value="user" ?selected=${roleSelected.has("user")}>${t("usage.userRole")}</option>
          <option value="assistant" ?selected=${roleSelected.has("assistant")}>${t("usage.assistantRole")}</option>
          <option value="tool" ?selected=${roleSelected.has("tool")}>${t("usage.toolRole")}</option>
          <option value="toolResult" ?selected=${roleSelected.has("toolResult")}>${t("usage.toolResultRole")}</option>
        </select>
        <select
          multiple
          size="4"
        ...
        <input
          type="text"
          placeholder="${t("usage.searchConversation")}"
          .value=${filters.query}
      ...
      <div class="session-logs-list">
        ...
        ${
          filteredEntries.length === 0
            ? html`
                <div class="muted" style="padding: 12px">${t("usage.noMatchingMessages")}</div>
              `
            : nothing
        }
      </div>
...
export function renderUsage(props: UsageProps) {
  ...
  return html`
    ...
    <section class="usage-page-header">
      <div class="usage-page-title">${t("usage.pageTitle")}</div>
      ...
      <div class="usage-header-row">
        <div class="usage-header-title">
          ...
          ${
            isEmpty
              ? html`
                  <span class="usage-query-hint">${t("usage.selectDateRange")}</span>
                `
              : nothing
          }
        </div>
            ...
            displayTotals
              ? html`
                <span class="usage-metric-badge">
                  <strong>${formatTokens(displayTotals.totalTokens)}</strong> ${t("usage.tokensUnit")}
                </span>
                <span class="usage-metric-badge">
                  <strong>${formatCost(displayTotals.totalCost)}</strong> ${t("usage.costUnit")}
                </span>
                <span class="usage-metric-badge">
      ...
      <div style="margin-top: 12px;">
          ...
          <div class="usage-query-actions">
            ...
            <span class="usage-query-hint">
              ${
                hasQuery
                  ? t("usage.sessionsMatch", {
                      count: filteredSessions.length,
                      total: totalSessions,
                    })
                  : t("usage.sessionsInRange", { count: totalSessions })
              }
            </span>
          </div>
        </div>
